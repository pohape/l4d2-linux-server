...
anonymous SteamCMD и кладёт её в `addons/workshop_<id>.vpk`. Идемпотентный —
если целевой VPK уже есть, скачивание пропускается (передай `FORCE=1`,
чтобы перекачать).

Возьми id из URL айтема
(`steamcommunity.com/sharedfiles/filedetails/?id=<WORKSHOP_ITEM>`) и запусти:

```bash
sudo WORKSHOP_ITEM=<id> bash /opt/l4d2-linux-server/scripts/install-workshop-map.sh
```

Имя файла (`workshop_<id>.vpk`) можно переопределить через `VPK_NAME=…`,
если хочешь более говорящее.
...
существующие файлы пропускаются, `FORCE=1` чтобы перекачать.
