...
Надёжный способ вернуться — перезапустить сервис:

```bash
sudo systemctl restart l4d2
```

Переподключишься с чистым team-assignment. Но это кикнет всех остальных,
кто подключён, так что используй только когда без вариантов.
