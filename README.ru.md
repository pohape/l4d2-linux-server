...
На `!botmenu` аддон ответит `This command is for admin only.`, пока
твоего SteamID нет в **его собственном** списке админов. SourceMod'овский
`admins_simple.ini` он не читает — тот отвечает только за команды `sm_*`.
Создай на сервере файл:

```bash
sudo -u steam tee "/home/steam/l4d2/left4dead2/ems/advanced bot ai/admins.txt" <<'EOF'
STEAM_1:0:12345678 //твой_ник
EOF
```

Формат — ровно такой, как его разбирает аддон:

- один SteamID в строке, в виде `STEAM_1:X:XXXXXXX`
- всё после `//` в строке игнорируется, туда удобно писать ник
- `STEAM_0` тоже принимается, аддон сам заменяет его на `STEAM_1`

Бери тот же SteamID, что вписан в `admins_simple.ini` (шаг 6). Если не
уверен, каким сервер видит именно тебя — зайди на сервер один раз и
посмотри в логе:
...
Список читается при инициализации аддона, поэтому после правки нужен
`sudo systemctl restart l4d2` или хотя бы смена карты.
